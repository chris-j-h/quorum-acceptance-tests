...
public class ContractService extends AbstractService {
    ...
    public Observable<? extends Contract> createPublicSimpleContract(int initialValue, Node source, String ethAccount) {
        return createSimpleContract(initialValue, QuorumNode.valueOf(source.getName()), ethAccount, null, DEFAULT_GAS_LIMIT, PrivacyFlag.STANDARD_PRIVATE);
    }

    public Observable<? extends Contract> createSimpleContract(int initialValue, Node source, String ethAccount, String privateFromAlias, List<String> privateForAliases, PrivacyFlag privacyFlag) {
        return createSimpleContract(initialValue, source, ethAccount, privateFromAlias, privateForAliases, privacyFlag, DEFAULT_GAS_LIMIT);
    }
    ...
    public Observable<? extends Contract> createSimpleContract(int initialValue, QuorumNode source, QuorumNode target, BigInteger gas) {
        ...
        return createSimpleContract(initialValue, source, null, targets, gas, PrivacyFlag.STANDARD_PRIVATE);
    }

    public Observable<? extends Contract> createSimpleContract(int initialValue, QuorumNode source, String ethAccount, List<QuorumNode> targets, BigInteger gas, PrivacyFlag privacyFlag) {
        Quorum client = connectionFactory().getConnection(source);
    ...
    public Observable<? extends Contract> createSimpleContract(int initialValue, QuorumNode source, String ethAccount, List<QuorumNode> targets, BigInteger gas, PrivacyFlag privacyFlag, List<QuorumNode> mandatoryFor) {
        Quorum client = connectionFactory().getConnection(source);
        final List<String> privateFor;
        if (null != targets) {
            privateFor = targets.stream().filter(q -> q != null).map(q -> privacyService.id(q)).collect(Collectors.toList());
    ...
    public Observable<TransactionReceipt> updateSimpleContractWithMandatoryRecipients(final QuorumNode source, List<QuorumNode> target,
                                                               final String contractAddress, final int newValue, PrivacyFlag flags, List<QuorumNode> mandatoryFor) {
    ...
    public Observable<TransactionReceipt> updateSimpleContract(final QuorumNode source, List<QuorumNode> target,
                                                               final String contractAddress, final int newValue, PrivacyFlag flags) {
    ...
    public Observable<? extends Contract> createGenericStoreContract(QuorumNetworkProperty.Node node, String contractName, int initalValue, String dpContractAddress, boolean isPrivate, QuorumNode target, PrivacyFlag privacyTypes) {
        Quorum client = connectionFactory().getConnection(node);
